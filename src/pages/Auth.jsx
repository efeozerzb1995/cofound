...
export default function Auth() {
  ...
  // Drive onboarding modal from AuthContext: only show when authenticated and flagged
  useEffect(() => {
  ...
  const handleSocialLogin = async (providerLabel) => {
    setLoadingProvider(providerLabel);
    setIsLoading(true);

    try {
      let supabaseProvider = null;
      if (providerLabel === 'Google') supabaseProvider = 'google';
      if (providerLabel === 'GitHub') supabaseProvider = 'github';

      if (!supabaseProvider) {
        toast.error(providerLabel + ' ile giriş henüz desteklenmiyor.');
        return;
      }
      ...
      const message = err && err.message ? err.message : 'Beklenmeyen bir hata oluştu.';
      toast.error(message);
    } finally {
      setIsLoading(false);
      setLoadingProvider('');
  ...
  const handleOnboardingComplete = async (data) => {
    try {
      ...
      const sessionUser = sessionData && sessionData.session ? sessionData.session.user : null;
      ...
      const profilePayload = {
        ...
        has_project_idea: data.hasProjectIdea ? true : false,
      };
      ...
      if (userId) {
        ...
        if (existingProfileError) {
          console.error(
            '[Auth] Onboarding existing profile fetch error:',
        ...
        console.warn(
          '[Auth] Onboarding complete but no Supabase userId found. Profile not saved to Supabase.',
        );
      }
      ...
      try {
        ...
        if (userId && skillsArray.length > 0) {
          ...
          const rows = skillsArray.map((name) => ({
            user_id: userId,
            name,
          }));
        ...
        console.error(
          '[Auth] Unexpected error while saving onboarding skills:',
          skillsError,
        );
      }
            ...
            >
              ...
              <blockquote className="space-y-6">
                <p className="text-3xl font-semibold leading-relaxed text-white">
                  Doğru ekibi bulmak, fikirleri hayata geçirme hızını katlıyor. CoFound ile
                  benzer hedeflere sahip insanlarla bir araya gelebilirsin.
                </p>
          ...
          >
            ...
            <div className="mb-8 text-center lg:text-left">
              ...
              <p className="text-slate-600 text-lg">
                {isSignUp
                  ? 'Ekibini bul, projeni hayata geçir'
